...
if (!conn.authState.creds.registered) {
  ...
  if (process.argv[2]) { // Changed process.argv[1] to process.argv[2]
            ...
            if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
                console.log(chalk.bgBlack(chalk.redBright("Masukkan nomor WhatsApp Anda. Contoh : 62xxx")));
                process.exit(0);
            }
        } else if (!global.pairingNumber) {
        ...
        setTimeout(async () => {
            ...
            spinnies.succeed('spinner-1', { text: `Pairing Code: ${code}`, successColor: "white"});
        }, 3000);
    }
}
...
async function connectionUpdate(update) {
    ...
    if (connection === "open") {
        ...
        const message = `• *Info*: ${global.namebot} telah aktif
...
◦ *Connected time*: ${new Date().toLocaleString()}`;
    ...
    if (connection == 'close') {
        console.log(chalk.yellow(`📡 Koneksi terputus dari server, mohon ambil ulang sessions bot`));
    }
}
...
setInterval(async () => {
    ...
    await clearTmp();
    conn.reply(info.nomerown + '@s.whatsapp.net', 'Tmp telah dibersihkan', null) >
        console.log(chalk.cyanBright(
            `\n╭───────────────────·»\n│\n` +

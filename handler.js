    ...
    try {
        ...
        try {
            ...
            if (user) {
                ...
                if (!isNumber(user.saham)) user.saham = 0
                    ...
                    if (e) {
                        ...
                        if (e.name)
                            for (let [jid] of global.owner.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
                                let data = (await conn.onWhatsApp(jid))[0] || {}
                                if (data.exists)
                                    m.reply(`「❗ *REPORT SYSTEM* ❗」
                                    
> Plugins : ${m.plugin}
> Sender : ${m.sender}
> Chat : ${m.chat}
> Chat : ${usedPrefix}${command} ${args.join(' ')}
〔 LOG 〕: 
...
> Untuk menon-aktifkan fitur ini, ketik
*.disable antidelete*
...
let msg = {
        ...
        premiumonly: `📢 Kamu tidak dapat mengakses fitur ❗\nBeli premium untuk mengakses lewat Private Chat.\nhubungi owner dibawah\nwa.me/${global.info.nomerown}`,
        groups: 'Saat ini fitur hanya aktif untuk grup saja. silahkan gunakan fitur bot di dalam grup',
        gconly: `📢 Kamu tidak dapat mengakses fitur khusus grup❗\n`,
  ...
  if (msg) return conn.sendMessage(m.chat, {
      ...
      contextInfo: {
      externalAdReply: {
      title: 'A K S E S  D I B A T A S I',
      body: 'KonexaMD',
      thumbnailUrl: akses,
      sourceUrl: ,
      mediaType: 1,
    ...
    let daftar = {
  unreg: `\`Anda belum terdaftar di Konexa. Segera daftar terlebih dahulu.\`
  
> .daftar nama.umur
> Klik Verify Di Bawah`
}[type];

if (daftar) return conn.sendMessage(m.chat, daftar, {quoted: fkon});
}

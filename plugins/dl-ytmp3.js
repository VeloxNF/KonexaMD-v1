...
let handlerMP3 = async (m, { conn, text, usedPrefix }) => {
  if (!text) throw `Contoh penggunaan: ${usedPrefix}ytmp3 <link atau judul YouTube>`;

  // Define quality and type for audio
  const qualityIndex = 3; // You can adjust this based on user input (e.g., 128kbps)
  const typeIndex = 1; // Audio type

  const result = await SaveTube.dl(text, qualityIndex, typeIndex);

  if (!result) throw `Gagal mengambil data audio dari YouTube!`;
...
handlerMP3.help = ['ytmp3', 'yta'];
handlerMP3.tags = ['downloader'];
handlerMP3.command = /^ytmp3|yta$/i;

export default handlerMP3;
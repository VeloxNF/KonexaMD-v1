  ...
  groups.forEach(x => {
    ...
    if (v) {
      rows.push({
        ...
        description: `[ ${v.stay ? 'FOREVER' : (v.expired == 0 ? 'NOT SET' : timeReverse(v.expired - new Date() * 1))}  | ${x.participants.length} | ${(v.isBanned ? 'OFF' : 'ON')} | ${moment(v.activity).format('DD/MM/YY HH:mm:ss')} ]`
      });
    }
  });
...
handler.tags = ['group'];
handler.command = /^(listgroups|listgroup|groupslist|grouplist)$/i;
handler.owner = true;

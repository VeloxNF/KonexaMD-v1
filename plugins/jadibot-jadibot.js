...
const handler = async (m, { conn, args, usedPrefix, command, isOwner, text }) => {
  ...
  const conns = global.conn;
  const user = global.db.data.users[m.sender];

  const authFile = `plugins/jadibot/${m.sender.split('@')[0]}`;
  const isInit = !fs.existsSync(authFile);
  ...
  // Pastikan simple.makeWASocket ada dan fungsional
  if (typeof simple.makeWASocket !== 'function') {
    console.error('makeWASocket is not a function. Please check your simple.js module.');
    return;
  }
  ...
  if (!conn.authState.creds.registered) {
    setTimeout(async () => {
      ...
      const image = await generateCanvas(hasilcode);
      const capt = `Enter the code in the image above to become a temporary bot.
